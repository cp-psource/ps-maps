...
class Agm_Fbnf_AdminPages {
	...
	public function register_settings() {
		add_settings_section(
			...
			__( 'Facebook Freunde in der Nähe', AGM_LANG ),
			'__return_false',
		...
		add_settings_field(
			...
			__( 'Richte die App ein', AGM_LANG ),
	...
	public function admin_notice () {
		...
		<div class="notice notice-warning">
			<p><?php
				echo '<b>' . esc_html(__('Hinweis:', AGM_LANG)) . '</b> ';
				echo sprintf(esc_html(
					__('Aufgrund der Ablehnung der erforderlichen Funktionen und Berechtigungen in der Facebook-API funktioniert die Erweiterung %s nicht für neu erstellte Anwendungen.', AGM_LANG)),
	...
	public function create_scope_box() {
		...
		<label for="agm-fbnf-radius">
			<?php _e( 'Suche nach Freunden innerhalb ', AGM_LANG ); ?>
		</label>
		<input type="number"
			id="agm-fbnf-radius"
			size="6"
		...
		<?php _e( 'Metern', AGM_LANG ); ?>

		<br />

		<label for="agm-fbnf-months">
			<?php _e( 'Suche nach Freunden in Updates und Fotos für Freunde in den letzten ', AGM_LANG ); ?>
		</label>
		<input type="text"
			id="agm-fbnf-months"
			size="3"
	...
	public function create_help_box() {
		?>
		<p>
			<?php _e( 'Befolge diese Schritte, um das Feld <em>App ID</em> einzurichten', AGM_LANG ); ?>
		</p>
		<ol>
			<li>
				<?php _e(
					'<a target="_blank" href="https://developers.facebook.com/apps">' .
					'Erstelle eine neue Facebook-App</a>', AGM_LANG
				); ?>
			</li>
			<li>
				<?php printf(
