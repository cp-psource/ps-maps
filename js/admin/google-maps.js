...
jQuery(function() {
    ...
    window.AgmMapHandler = function(selector, data, allowInsertion) {
                ...
                function(data) {
                    ...
                    if (!data.status) {
                        ...
                        jQuery(document).trigger(
                            'agm-map-saved', [parseInt(data.id)]
                        );

                        // Legacy trigger
                        container.trigger(
                            'agm_map_saved', [parseInt(data.id)]
                        );
        ...
        function createMarkup() {
             ...
             */
            jQuery(document).trigger(
                'agm_google_maps-admin-markup_created', [container.find('.agm-preview'), data]
            );
             ...
             */
            jQuery(document).trigger(
                'agm_google_maps-admin-options_initialized', [container.find('.agm-options'), data]
            );
            ...
            jQuery(document).trigger(
                'agm_google_maps-admin-map_resized', [container, data]
            );
        }
        ...
        function populate_map_options() {
            ...
            jQuery('#agm_map_size_associate').on("click", togglePostAssociate);
        }
        ...
        function showMarkerIconsList() {
            ...
            jQuery.post(
                window.ajaxurl, { "action": "agm_list_icons" },
                function(data) {
                    var html = '';
        ...
        function init() {
            ...
            jQuery('#agm_map_drop_marker').on("click", dropNewMarker);
            jQuery('.agm_go_back').on("click", destroyMap);
            jQuery('#agm_insert_map').on("click", insertMap);
            jQuery('.agm_map_options_open').on("click", toggleOptions);
            jQuery('.agm_map_options_close').on("click", toggleOptions);
            jQuery('.agm_save_map').on("click", saveMap);
            jQuery('#agm_mh_location_trigger').on("click", searchNewLocation);
            jQuery('#agm_map_size_default').on("click", toggleDefaultSize);
