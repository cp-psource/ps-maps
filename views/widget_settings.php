...
<p class="agm_widget_query_options">
	<label><?php _e( 'Verwende Karten von:', AGM_LANG ); ?></label>
	<br />
	...
	<select class="map_id_target" id="<?php echo $this->get_field_id( 'map_id' ); ?>" name="<?php echo $this->get_field_name( 'map_id' ); ?>">
		<option value=""><?php _e( 'Wähle ein', AGM_LANG ); ?></option>
	...
	<?php if (AGM_USE_POST_INDEXER) { // Integrate post indexer ?>
		<input class="post_indexer_target" type="checkbox" id="<?php echo $this->get_field_id( 'network' ); ?>" name="<?php echo $this->get_field_name( 'network' ); ?>" <?php echo (( 'network' == $network) ? 'checked="checked"' : '' ); ?> value="network" />
			<label for="<?php echo $this->get_field_id( 'network' ); ?>"><?php _e( 'Dies ist eine Netzwerkabfrage <small>(Benötigt <a href="https://n3rds.work/piestingtal-source-project/ps-multisite-index/">PS-Multisite Beitragsindex</a> plugin) - nur "tag = ..." wird unterstützt</small>', AGM_LANG ); ?></label>
			<br />
	<?php } ?>
	<small><em><a target="_blank" href="https://n3rds.work/piestingtal-source-project/ps-gmaps/"><?php _e( 'Weitere Informationen zu benutzerdefinierten Abfragen', AGM_LANG ); ?></a></em></small>
</p>

<p>
	<label><?php _e( 'Zeige:', AGM_LANG ); ?></label>
	<br />
	...
	<input id="<?php echo $this->get_field_id( 'show_markers' ); ?>" name="<?php echo $this->get_field_name( 'show_markers' ); ?>" <?php echo (($show_markers) ? 'checked="checked"' : '' ); ?> type="checkbox" value="1" />
		<label for="<?php echo $this->get_field_id( 'show_markers' ); ?>"><?php _e( 'Markierungsliste anzeigen', AGM_LANG ); ?></label>
	<br />
	<input id="<?php echo $this->get_field_id( 'show_images' ); ?>" name="<?php echo $this->get_field_name( 'show_images' ); ?>" <?php echo (($show_images) ? 'checked="checked"' : '' ); ?> type="checkbox" value="1" />
		<label for="<?php echo $this->get_field_id( 'show_images' ); ?>"><?php _e( 'Bilderstreifen anzeigen', AGM_LANG ); ?></label>
	<br />
